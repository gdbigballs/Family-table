...
- **移动端后台重做**：底部 Tab 栏（概览 / 订单 / 菜单 / 更多）+ 更多抽屉，不再横向滚动 8 个 tab；订单列表、菜单管理在手机上改为卡片式布局。
- **分类管理升级**：分类改名失焦自动保存、排序本地调整后统一保存、支持删除分类（有菜品的分类点击会明确提示）；分类筛选与管理收进抽屉。
- **后台操作局部刷新**：切换页面、处理订单、维护菜单不再整页重建，滚动位置与输入焦点不丢失。
- **营业状态卡重排**：状态圆点内联在"营业中"旁，并新增"调整预约与营业"直达入口。
- **代码重构**：清理约 500 行历史死代码、展开函数覆盖链；修复新增菜品时规格属性无法保存的历史 bug。
- **功能调整**：移除已无实际用途的管理员备注；统一订单状态机（移除已废弃的"制作中 / 已取消"）。
- **无障碍**：后台导航补全 ARIA 语义与键盘焦点样式。

详细更新内容见 [v0.3.0 Release Notes](docs/releases/v0.3.0.md)。

## v0.2.5

本版本在"关于项目"页面新增**更新日志**：点击版本号即可弹出更新日志弹窗，展示 GitHub 上各版本 Release 的发布说明（与"检查更新"卡片同款 Markdown 渲染），并可直接跳转到 GitHub 查看。

详细更新内容见 [v0.2.5 Release Notes](docs/releases/v0.2.5.md)。

## v0.2.4

本版本为"检查更新"弹窗中的发布说明增加 Markdown 渲染：新增零依赖的轻量渲染器（支持标题、列表、粗体、行内代码、代码块、链接），内容先转义再渲染、链接仅允许 http/https，防止注入；Release Notes 不再以纯文本展示，标题、列表、代码块等样式更清晰。

详细更新内容见 [v0.2.4 Release Notes](docs/releases/v0.2.4.md)。

## v0.2.3

本版本为下单与点评接口加入限流与幂等保护，防止脚本刷单导致数据库膨胀和企业微信通知轰炸：下单接口同一 IP 10 分钟内最多 3 单，超限返回 429 并提示等待时间；前端提交订单时附带一次性 `requestId`，同一编号 15 分钟内重复提交只处理一次（防双击 / 网络重试重复下单）。点评限流改用通用滑动窗口实现（15 分钟 3 条不变），并支持解析 `X-Forwarded-For` 获取真实 IP、自动清理过期计数，避免内存无限增长。

详细更新内容见 [v0.2.3 Release Notes](docs/releases/v0.2.3.md)。
...
├─ docs/releases/      # 各版本发布说明（v0.1.2.md 至 v0.3.0.md）
