...
本版本围绕实际点菜流程优化了首页和移动端体验：手机端菜单分类会固定在左侧并随滚动自动高亮，已选菜品改为底部操作栏，确认订单页可在“立即点菜”和“预约用餐”之间切换。

详细更新内容见 [v0.1.2 Release Notes](RELEASE_NOTES_v0.1.2.md)。

## 页面预览

### 桌面端菜单

![桌面端菜单](public/assets/docs/menu-desktop-v0.1.2.png)

桌面端使用左侧分类导航和多列菜品展示，底部提供已选菜品入口与确认按钮。

### 移动端菜单

<img src="public/assets/docs/menu-mobile-v0.1.2.png" alt="移动端菜单" width="320">

移动端分类栏会在进入菜单区后固定在左侧，滚动到最后一类菜时仍可直接切换分类。

### 移动端确认订单

<img src="public/assets/docs/checkout-mobile-v0.1.2.png" alt="移动端确认订单" width="320">

确认订单页支持在立即点菜和预约用餐两种方式间切换；预约方式会显示日期、时段和人数填写项。

一个面向家庭和小型聚餐场景的轻量点菜、预约与订单管理服务。访客无需注册即可浏览菜单、选择规格、提交点单；管理员在同一个站点中维护菜单、处理订单，并可在新订单到来时接收企业微信通知。

项目刻意不包含价格、支付、收银、库存和多角色权限，适合单人维护并部署在 NAS 或一台小服务器上。
...
项目当前没有第三方 npm 依赖，因此不需要执行 `npm install`。
...
请不要将简单密码用于 NAS 或公网环境。管理员密码可随时在“站点设置 → 密码设置”中修改。
...
Webhook 地址包含机器人密钥，请不要发送给他人，也不要写入 GitHub Issue、截图或代码仓库。
...
不要把 `data/` 作为公开 SMB 共享，也不要直接上传到 GitHub。项目的 `.gitignore` 已忽略 `data/`、`.env` 和 `node_modules/`。
